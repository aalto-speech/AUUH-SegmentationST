...
lang="$1"
task="$2"
expdir=${EXPOUT}/${lang}.${task}.flatcat_unsup/${RUN_FAMILY}-${SEED}

mkdir -p $expdir

### STAGE 1: Preprocess data ###
if [ $stage -le 1 ]; then
  if [ -d ${PROCESSED}/${lang}.${task}.flatcat_unsup ]; then
    echo "Using existing preprocessed data in ${PROCESSED}/${lang}.${task}.flatcat_unsup"
  else
    echo "Preprocessing data to ${PROCESSED}/${lang}.${task}.flatcat_unsup"
    mkdir -p ${PROCESSED}/${lang}.${task}.flatcat_unsup
    if [ ${task} == "sentence" ]; then
      python3 char_tokenize.py \
        ${AUGMENTROOT}/${lang}_best_flatcat_unsup.sentence.train.tsv \
        ${PROCESSED}/${lang}.${task}.flatcat_unsup/train
      python3 char_tokenize.py \
        ${AUGMENTROOT}/${lang}_best_flatcat_unsup.sentence.dev.tsv \
        ${PROCESSED}/${lang}.${task}.flatcat_unsup/dev
    else
      python3 char_tokenize.py \
        ${AUGMENTROOT}/${lang}_best_flatcat_unsup.train.tsv \
        ${PROCESSED}/${lang}.${task}.flatcat_unsup/train
      python3 char_tokenize.py \
        ${AUGMENTROOT}/${lang}_best_flatcat_unsup.dev.tsv \
        ${PROCESSED}/${lang}.${task}.flatcat_unsup/dev
    fi
  fi
fi
...
if [ $stage -le 2 ]; then
  marian \
    ...
    --train-sets \
      ${PROCESSED}/${lang}.${task}.flatcat_unsup/train.src.txt \
      ${PROCESSED}/${lang}.${task}.flatcat_unsup/train.tgt.txt \
    --vocabs \
      ${PROCESSED}/${lang}.${task}.flatcat_unsup/train.src.vocab \
      ${PROCESSED}/${lang}.${task}.flatcat_unsup/train.tgt.vocab \
    --valid-sets \
      ${PROCESSED}/${lang}.${task}.flatcat_unsup/dev.src.txt \
      ${PROCESSED}/${lang}.${task}.flatcat_unsup/dev.tgt.txt \
    --seed ${SEED} \
...
if [ $stage -le 3 ]; then
  marian-decoder \
    ...
    --input ${PROCESSED}/${lang}.${task}.flatcat_unsup/dev.src.txt \
